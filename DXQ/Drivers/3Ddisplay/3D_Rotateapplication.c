...
void RotatePic32X32(unsigned char *dp,float ax,float ay,float az,GUI_COLOR color,uint16_t XO,uint16_t YO)
{
//	int16_t  XO;
//	int16_t  YO;
	uint8_t i,j,k;
	...
	//Scale_3D(gMAT,8,8,8);		 //比例变换矩阵
	Rotate_3D(gMAT,ax,ay,az);	//旋转变换矩阵
	Translate3D(gMAT,16,16,32); 	//平移变换矩阵	   x:调节距离中心点的位置，相当于下面Point0.z
					//		  y：上下调动位置 ，具体根据实际修改
//	XO=128;
//	YO=64;
		...
		for(k=0;k<8;k++)
		{
			...
			for(j=0;j<32;j++)
			{
				if(*(dp+j)&temp)
				{
					Point0.x=j;
					Point0.y=(i*8)+k;
					Point0.z=-24;		//此参数能够改变字符距离旋转轴中心的距离
